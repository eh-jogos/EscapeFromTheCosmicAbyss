...
func _ready():
	...
	if OS.has_feature("demo") or get_tree().get_current_scene() == owner:
		for index in get_child_count():
			...
			button.focus_neighbor_bottom = button.get_path_to(_button_buy_game)
			
			if button.name == "TutorialButton":
				button.focus_neighbor_left = button.get_path_to($LevelButton5)
			elif button.name == "LevelButton5":
				button.focus_neighbor_right = button.get_path_to(_button_buy_game)
				button.focus_neighbor_top = button.get_path_to(_button_buy_game)
