...
do
	...
	godot godot_version --export $unquoted_profile $final_path_debug$unquoted_filename
	
	final_path_debug=$base_builds_path$version"/Debug/"$project_name$unquoted_profile"/"
	mkdir -p $final_path_debug
	godot godot_version --export-debug $unquoted_profile $final_path_release$unquoted_filename
done